...
packages/contracts
  └─ shared Zod profile and API response contracts
```

The browser owns interactive authentication through the official Supabase SDK. The API owns profile HTTP policy and never trusts identity fields from the browser. PostgreSQL RLS independently enforces the same ownership rule.

## Workspace boundaries

- `apps/web` owns public/authenticated routes, auth screens, the in-memory current access token, and client-side form validation.
- `apps/api` verifies bearer access tokens, derives `userId`, validates API input, and accesses profiles through a user-scoped Supabase client.
- `packages/contracts` contains shared Zod schemas for profile request/response shapes.
- `supabase/migrations` is the version-controlled database source of truth.
- `supabase/tests/database` tests profile provisioning and RLS independently of Express.

React Context contains only session identity and auth actions. TanStack Query owns the remote profile. This prevents Context from becoming a second, stale server-state cache.
...
`/app`, `/app/onboarding`, and `/app/profile` require a restored Supabase session. After identity is known, the profile query decides routing:
...
- Neither app accepts a `userId` for profile ownership. The verified token is the only identity source.
...
Registration, login, confirmation, resend, and recovery calls go directly to Supabase and use its provider rate limits. Harap-owned profile endpoints remain behind the Phase 1 API limit of 100 requests per IP per minute. A duplicate login rate limiter in Express would not cover the actual Auth endpoint; future expensive endpoints will receive route-specific limits.
